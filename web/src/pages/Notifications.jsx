      ...
      {isAdmin ? (
        <>
          <div className="mt-4 flex flex-wrap gap-2">
            <TabButton active={tab === 'needs'} onClick={() => setTab('needs')} icon={CheckCircle2}>Needs action</TabButton>
            <TabButton active={tab === 'all'} onClick={() => setTab('all')} icon={Bell}>All activity</TabButton>
            <TabButton active={tab === 'mine'} onClick={() => setTab('mine')} icon={User}>Mine</TabButton>
          </div>
          {tab === 'needs' ? <NeedsAction /> : tab === 'all' ? <ActivityList admin /> : <ActivityList />}
        </>
      ) : (
        <ActivityList />
      )}
            ...
            <li key={`${it.item_type}-${it.ref_id}`} className="flex flex-wrap items-center gap-3 p-4">
              ...
              {it.item_type === 'success' ? (
                <div className="flex shrink-0 gap-2">
                  <button className="btn-primary px-3 py-1.5 text-xs" disabled={busy === it.ref_id} onClick={() => reviewSuccess(it.ref_id, true)}>Approve</button>
                  <button className="btn-outline px-3 py-1.5 text-xs" disabled={busy === it.ref_id} onClick={() => reviewSuccess(it.ref_id, false)}>Reject</button>
                  <button className="btn-outline px-2.5 py-1.5 text-xs" title="Open post" onClick={() => navigate(`/post/${it.ref_id}`)}><ExternalLink size={14} /></button>
                </div>
              ) : (
                <button className="btn-outline px-3 py-1.5 text-xs" onClick={() => navigate(it.item_type === 'solution' ? `/problem-hub/${it.ref_id}` : `/pipeline/${it.ref_id}`)}>Review</button>
              )}
            </li>
          ))}
        </ul>
              ...
              <li key={n.id}>
                ...
                {open && (
                  <div className="flex flex-wrap items-center gap-2 border-t border-line bg-page/60 px-4 py-3">
                    {target && (
                      <button className="btn-outline inline-flex items-center gap-1.5 px-3 py-1.5 text-xs" onClick={() => openTarget(n)}>
                        <ExternalLink size={13} /> Open {n.idea_id ? 'application' : 'post'}
                      </button>
                    )}
                    {!admin && !n.read_at && (
                      <button className="btn-outline inline-flex items-center gap-1.5 px-3 py-1.5 text-xs" onClick={() => markRead(n)}>
                        <Check size={13} /> Mark read
                      </button>
                    )}
                    {!admin && (
                      <button className="btn inline-flex items-center gap-1.5 border border-down/40 px-3 py-1.5 text-xs text-down hover:bg-down/10" onClick={() => remove(n)}>
                        <Trash2 size={13} /> Delete
                      </button>
                    )}
                    {!target && admin && <span className="text-xs text-faint">No linked item.</span>}
                  </div>
                )}
              </li>
            )
